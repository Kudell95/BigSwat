...
public class Pure_FPP_Controller : MonoBehaviour {
	...
	void Update()
	{
		...
		}else{ //if crouch toggle is selected
			if(Input.GetButtonDown (CrouchInput)){
				CrouchFlag = !CrouchFlag;
			}	
			// Debug.Log(CrouchFlag);

				if ((CrouchFlag == true || CrouchingBlocked) && Controller.isGrounded ) { //crouch
				...
				if (CrouchFlag == false && CrouchingBlocked == false) {  //stand up after crouching
					if (CharacterCamera != null) {
						CameraVector = new Vector3 (CharacterCamera.transform.localPosition.x, CamPos.y, CharacterCamera.transform.localPosition.z);
						CharacterCamera.transform.localPosition = CameraVector;
					}
	...
	void FixedUpdate()
	{
		...
		if (!Input.GetButton(CrouchInput) && CrouchingBlocked == false && !Input.GetButton(RunInput) && !CrouchFlag) {
		...
		if (Input.GetButton (CrouchInput) || CrouchingBlocked || CrouchFlag) {
